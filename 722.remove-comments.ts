...
function removeComments(source: string[]): string[] {
    ...
    {
        let line: string = source[x];
        let tempLine: string = line;
        
        // Check for multi line initiator 
        if (line.indexOf('/*') > -1)
        {
            // make sure that single line comment is not before initiator
            if (!(line.indexOf('//') > -1 && line.indexOf('//') < line.indexOf('/*')))
            {
                tempLine = ""
                do {
                    // capture text before initiator
                    tempLine += line != undefined ? line.substr(0, line.indexOf('/*')) : '';
                    line = line != undefined ? line.substr(line.indexOf('/*') + 2) : '';
                    // skip lines until terminator is found
                    while(line != undefined && line.indexOf('*/') <= -1)
                    {
                        x++
                        line = source[x];
                    }
                    // capture text after initiator
                    tempLine += line != undefined ? line.substr(line.indexOf('*/') + 2) : '';
                    // repeat if more initiators
                    if (tempLine.indexOf('/*') <= -1) break;
                } while (true)
            }
        }
        ...
        if (tempLine.indexOf('//') > -1)
            tempLine = tempLine.substr(0, tempLine.indexOf('//'));

        // Add the line to the output if not empty
        if (tempLine != "")
...
var test = removeComments(source)
console.log('test')
